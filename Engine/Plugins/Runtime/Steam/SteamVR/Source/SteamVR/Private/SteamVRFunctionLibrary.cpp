...
// MIXED-REALITY
void USteamVRFunctionLibrary::SetCaptureRenderTarget(UTextureRenderTarget2D* NewTexture)
{
	FSteamVRHMD* SteamVRHMD = GetSteamVRHMD();
	if (SteamVRHMD)
	{
		SteamVRHMD->SetCameraRenderTarget(NewTexture);
	}
}

void USteamVRFunctionLibrary::RemoveCaptureRenderTarget(UTextureRenderTarget2D* NewTexture)
{
	FSteamVRHMD* SteamVRHMD = GetSteamVRHMD();
	if (SteamVRHMD)
	{
		SteamVRHMD->RemoveCameraRenderTarget(NewTexture);
	}
}
